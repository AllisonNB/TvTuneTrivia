...
const Input = styled.input`
  ...
  width: 200px;
  ${(props) =>
    props.$numOfGuesses > 0 &&
    `border: ${props.$isCorrect ? "4px solid green;" : "4px solid red;"}}`}
`;
...
export default function Guesser({
  ...
  const evaluateGuess = () => {
    ...
    const wordsRegex = new RegExp(
      "\\b(" + removedWords.join("|") + ")\\b",
      "gi"
    );
    const parenthesesRegex = /\(.*?\)/gi;

    const editedTrack = trackName
      ?.replace(wordsRegex, "")
    ...
    const commonWords = editedTrack?.filter((word) =>
      editedGuess.includes(word)
    );
  ...
  return (
    <FormContainer action="submit">
      ...
      <Feedback aria-live="polite">
        ...
        />
        {isCorrect && numOfGuesses > 0 ? (
          <p aria-label="feedback">Correct!</p>
        ) : null}
        {!isCorrect && numOfGuesses > 0 ? (
          <p aria-label="feedback">Incorrect!</p>
        ) : null}
      </Feedback>
      <div>
